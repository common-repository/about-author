 ...
 * Version: 1.6.2
 * Description: Display Blog Authors Information In Style you can publish blog users profile into any Page or Post in your WordPress sites. You can use multiple shortcode on multiple pages and posts to display author profiles in various combinations of customization. You can use multiple widgets to display author profiles into any sidebar area or footer widget area.
 * Author: Weblizar
...
class About_Author_Shorcode_And_Widget
{
	...
	public function Weblizar_Save_fag_meta_box_save($PostID)
	{
		if (isset($_POST['About_me_user_name']) && isset($_POST['About_me_web_site_name']) && isset($_POST['About_me_web_site_url'])) {
			...
			$About_me_user_name     = sanitize_text_field($_POST['About_me_user_name']);
			$About_me_user_name     = sanitize_text_field( $About_me_user_name);
			...
			$About_me_dis_cription  = sanitize_text_field($_POST['About_me_dis_cription']);
			$About_me_dis_cription  = sanitize_text_field($About_me_dis_cription);
...
add_action('media_buttons', 'aa_add_rpg_custom_button');
add_action('admin_footer', 'aa_add_rpg_inline_popup_content');

//add media button fuction
function aa_add_rpg_custom_button()
{
	$context	  =  null;
	$container_id = 'AMSA';
	$title        = 'About Author';
	$context      .= '<a class="button button-primary thickbox" title="'.$title.'" href="#TB_inline?width400&inlineId=' . $container_id . '"><span style="margin-top:5px;" class="dashicons dashicons-id"></span> About Author</a>';
	printf($context);
}
...
{ ?>
	...
	<div id="AMSA" style="display:none;">
		...
		if (!$all_posts == null) { ?>
			<h5><?php esc_html_e('Select About Author Shortcode And Widget To Insert Into Post', 'WEBLIZAR_ABOUT_DOMAIN'); ?></h5>
			<select id="Ab_Tm_ME">
				<?php
				global $wpdb;
...
function load_author_info_after_post_content($content)
{
	if (is_single() && get_post_type($post = get_post()) == "post") {
		...
		if(isset($Weblizar_io_settings[0]['Author_short_code'])){
