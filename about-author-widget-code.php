	...
	*/
	public function form( $instance ) {
		...
		<p>
			...
			$weblizar_post_arr_about_author = json_decode(json_encode(wp_count_posts( $Weblizar_CPT_Name )), true);
			if($weblizar_post_arr_about_author != NULL){
			...
			<select id="<?php echo esc_attr($this->get_field_id( 'Shortcode' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'Shortcode' )); ?>" style="width: 100%;">
				<option value="<?php echo esc_attr('Select Any About Author Shortcode');?>" <?php if($Shortcode == "Select Any About Author Shortcode") echo 'selected="selected"'; ?>><?php esc_html_e( 'Select Any About Author Shortcode', 'WEBLIZAR_ABOUT_DOMAIN' ); ?></option>
				<?php
